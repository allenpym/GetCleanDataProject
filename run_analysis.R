setwd("F:/study/Coursera/GetCleanData/Project")
...
# Obtain feature names from featurex.txt
...
# find column index for mean and std to extract
...
data$activity<-factor(data$activity,
                      labels=act_label$activity,
                      levels=act_label$label)
# write.table(data, file = "data.txt")
# dat<-read.table("data.txt",header=T)


# creat data2 as independent tidy data set with 
...
write.table(data2.T, file = "data2.txt", row.name=FALSE)
# dat<-read.table("data2.txt",header=T)
# head(dat)
